 ...
 */
export async function saveWrongAnswersToCloud(userId: string, wrongAnswers: WrongAnswer[]): Promise<void> {
  try {
 ...
 */
export async function loadWrongAnswersFromCloud(userId: string): Promise<WrongAnswer[] | null> {
  try {
    const wrongAnswersRef = doc(db, 'expression_wrong_answers', userId);
    const docSnap = await getDoc(wrongAnswersRef);
    if (docSnap.exists()) {
      const data = docSnap.data();
      return (data.list || []) as WrongAnswer[];
    }
    return null;
  } catch (error: any) {
