...
import { Award, Check, X, RefreshCw, Trash2, CheckCircle2, CheckSquare, Square } from 'lucide-react';
...
export const ReviewRoom: React.FC<ReviewRoomProps> = ({
  ...
  const handlePushSingleToMochi = async (wa: WrongLabAnswer) => {
  ...
  return (
    <div className="glass-panel main-panel animate-fade-in" style={{ padding: '2rem' }}>
      ...
      {displayedAnswers.length === 0 ? (
        ...
        <div className="review-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.25rem' }}>
          {displayedAnswers.map((wa) => {
            ...
            return (
              <div key={wrongId} className="glass-panel review-card" style={{ background: 'rgba(255, 255, 255, 0.01)', borderColor: isCorrect ? 'var(--success)' : 'var(--border-color)', position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '1.25rem', gap: '1rem' }}>
                
                {/* Checkbox overlay */}
                <div 
                  style={{ position: 'absolute', top: '1.25rem', left: '1.25rem', cursor: 'pointer', zIndex: 10 }}
                ...
                <div style={{ paddingLeft: '1.8rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end', alignItems: 'center', flexWrap: 'wrap' }}>
                    <button
                      ...
                      onClick={() => handlePushSingleToMochi(wa)}
