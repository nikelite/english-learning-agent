...
export async function saveSharedLessonProgress(
  ...
  try {
    ...
    const ref = doc(db, 'lab_shared_progress', docId);
    await setDoc(ref, {
      lessonId,
      userId,
      progress,
...
export async function loadSharedLessonsProgress(
  ...
  try {
    ...
    const q = query(collection(db, 'lab_shared_progress'), where('userId', 'in', variations));
...
export async function syncUserLessons(userId: string, localLessons: LabLesson[]): Promise<LabLesson[]> {
  try {
    ...
    const qOwner = query(collection(db, 'lab_lessons'), where('ownerId', 'in', variations));
    ...
    const qShared = query(collection(db, 'lab_lessons'), where('sharedWith', 'array-contains-any', variations));
    ...
    // 5. Add remaining cloud lessons that weren't in local history
 ...
 */
export async function saveWrongAnswersToCloud(userId: string, wrongAnswers: WrongLabAnswer[], updatedAt?: number): Promise<void> {
  try {
 ...
 */
export async function loadWrongAnswersFromCloud(userId: string): Promise<{ list: WrongLabAnswer[], updatedAt: number } | null> {
  try {
    const wrongAnswersRef = doc(db, 'lab_wrong_answers', userId);
    ...
    if (docSnap.exists()) {
      const data = docSnap.data();
      return {
        list: (data.list || []) as WrongLabAnswer[],
