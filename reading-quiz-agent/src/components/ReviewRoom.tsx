...
export const ReviewRoom: React.FC<ReviewRoomProps> = ({
  ...
  const handlePushSingleToMochi = async (wa: WrongReadingAnswer) => {
      ...
      {displayedAnswers.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.25rem' }}>
          {displayedAnswers.map((wa) => {
            ...
            return (
              <div key={wrongId} className="glass-panel" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '1rem', background: 'rgba(255, 255, 255, 0.01)', borderColor: isCorrect ? 'var(--success)' : 'var(--border-color)', position: 'relative' }}>
